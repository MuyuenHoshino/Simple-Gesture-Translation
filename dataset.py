...
class CSL_Continuous_Char(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            print(idx%self.videos_per_folder)
            selected_folder = selected_folders[idx%self.videos_per_folder]
        else:
            selected_folder = selected_folders[idx%self.videos_per_folder + int(0.8*self.signers*self.repetition)]
        images = self.read_images(selected_folder)
...
class CSL_Continuous_Mediapipe(Dataset):
    def \
            __init__(self, data_path, dict_path, corpus_path, frames=48, mode="train"):
        super(CSL_Continuous_Mediapipe, self).__init__()
        # 3个路径
        self.data_path = data_path
        self.dict_path = dict_path
        self.corpus_path = corpus_path
        ...
        if self.mode=="train":
            ...
            self.videos_per_folder = int(0.15 * self.signers * self.repetition)
        elif mode=="test":
            self.videos_per_folder = int(0.1 * self.signers * self.repetition)
        else:
            print("mode error")
        # dictionary
        self.dict = {'<pad>': 0, '<sos>': 1, '<eos>': 2}    # 补全、句首、句尾
    ...
    def __getitem__(self, idx):
        ...
        # selected_folders就是文件夹内全部视频的路径
        selected_npy = [os.path.join(top_folder, item) for item in os.listdir(top_folder)]
        # sorted可以对所有可迭代的对象进行排序操作，但是结果表明此列表不可迭代
        # selected_folders = sorted([item for item in selected_folders_s if os.path.isdir(item)])
        ...
        # 根据索引选定一个视频文件
        if self.mode=="train":
            selected_npy = selected_npy[idx%self.videos_per_folder]
        elif self.mode=="val":
            selected_npy = selected_npy[idx%self.videos_per_folder + int(0.75*self.signers*self.repetition)]
        else:
            selected_npy = selected_npy[idx%self.videos_per_folder + int(0.9*self.signers*self.repetition)]
...
class CSL_Isolated_mediapipe(Dataset):
    def __init__(self, keypoints_path, label_path, num_classes=500, mode="train"):
        super(CSL_Isolated_mediapipe, self).__init__()
        ...
        if self.mode == "train" :
            ...
            self.videos_per_folder = int(0.10 * self.signers * self.repetition)
        else:
            print("mode error")
            raise
        self.labels = {}
    ...
    def __getitem__(self, idx):
        #print("get_getitem:",self.data_folder)
        s = "%03d" % int(idx / self.videos_per_folder)
        top_folder = os.path.join(self.keypoints_path ,s)
        selected_folders = [os.path.join(top_folder, item) for item in os.listdir(top_folder)]
        #print(selected_folders)
        if self.mode == "train":
            #print(idx%self.videos_per_folder)
            selected_npy = selected_folders[idx%self.videos_per_folder]
        elif self.mode == "val":
            selected_npy = selected_folders[idx%self.videos_per_folder + int(0.75*self.signers*self.repetition)]
        else:
            selected_npy = selected_folders[idx % self.videos_per_folder + int(0.9 * self.signers * self.repetition)]
        ...
        label = torch.LongTensor([int(idx/self.videos_per_folder)])
        print(idx)
        return {'data': sequence, 'label': label}
...
class CSL_Continuous_TwoStream(Dataset):
    def \
            __init__(self,pic_path,data_path, dict_path, corpus_path, frames=48, mode="train",transform=None):
        super(CSL_Continuous_TwoStream, self).__init__()
        # 3个路径
        self.data_path = data_path
    ...
    def __getitem__(self, idx):
        ...
        s = "%06d" % int(idx/self.videos_per_folder)
        top_folder = os.path.join(self.data_path, s)
        pic_top_folder =  os.path.join(self.pic_path, s)
        # top_folder = self.data_folder[int(idx/self.videos_per_folder)]
        # top_folder 'D:/Download/CSL_Continuous/color\\000005'
        # os.listdir 用于返回指定的文件夹包含的文件或文件夹的名字的列表
        ...
        # selected_folders就是文件夹内全部视频的路径
        selected_npy = [os.path.join(top_folder, item) for item in os.listdir(top_folder)]
        selected_pic_folders = [os.path.join(pic_top_folder, item) for item in os.listdir(pic_top_folder)]
        # sorted可以对所有可迭代的对象进行排序操作，但是结果表明此列表不可迭代
        # selected_folders = sorted([item for item in selected_folders_s if os.path.isdir(item)])
        ...
        if self.mode=="train":
            selected_npy = selected_npy[idx%self.videos_per_folder]
            #selected_pic_folder = selected_pic_folders[idx%self.videos_per_folder]
        elif self.mode=="val":
            selected_npy = selected_npy[idx%self.videos_per_folder + int(0.75*self.signers*self.repetition)]
            #selected_pic_folder = selected_pic_folders[idx%self.videos_per_folder + int(0.75*self.signers*self.repetition)]
        else:
            selected_npy = selected_npy[idx%self.videos_per_folder + int(0.9*self.signers*self.repetition)]
            #selected_pic_folder = selected_pic_folders[idx%self.videos_per_folder + int(0.9*self.signers*self.repetition)]
        
        npy_basename = os.path.splitext(os.path.basename(selected_npy))[0]

        for selected_pic_folder in selected_pic_folders:
            pic_basename = os.path.basename(selected_pic_folder)
            #print(pic_basename,npy_basename)
            if pic_basename == npy_basename:
                break

        print(selected_npy,selected_pic_folder)
