...
from models.Seq2Seq import Encoder, Decoder, Seq2Seq,Myencoder_mediapipe,Mediapipe_Transformer,TwoStream_Transformer, CSL_Daily_TwoStream_Transformer,CSL_Daily_TwoStream_Transformer_real
...
log_path = "log/CSL-Daily_en-de_{:%Y-%m-%d_%H-%M-%S}.log".format(datetime.now())
sum_path = "runs/CSL-Daily_en-de_{:%Y-%m-%d_%H-%M-%S}".format(datetime.now())
...
if __name__ == '__main__':
    ...
    decoder = Decoder(output_dim=7753, emb_dim=emb_dim, enc_hid_dim=enc_hid_dim, dec_hid_dim=dec_hid_dim, dropout=dropout).to(device)
